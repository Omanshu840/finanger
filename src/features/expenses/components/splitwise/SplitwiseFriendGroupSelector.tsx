...
export function SplitwiseFriendGroupSelector({
  ...
  const groupsQuery = useQuery({
    queryKey: splitwiseQueryKeys.groups(),
    queryFn: () => splitwiseClient.getGroups(),
    staleTime: 10 * 60 * 1000,
  })

  const friendsQuery = useQuery({
    queryKey: splitwiseQueryKeys.friends(),
    queryFn: () => splitwiseClient.getFriends(),
    staleTime: 10 * 60 * 1000,
  })

  const groups = groupsQuery.data ?? []
  const friends = friendsQuery.data ?? []
  const loading = groupsQuery.isLoading || friendsQuery.isLoading
