...
import {
  ...
  type GroupMember,
} from '@/features/splitwise/api/splitwiseApi'
...
export function SplitwiseExpenseForm({
  ...
  const loadParticipants = async () => {
    try {
      ...
      if (entity.type === 'group') {
        ...
        const groupDetails = await queryClient.fetchQuery({
          queryKey: splitwiseQueryKeys.group(group.id),
          queryFn: () => splitwiseClient.getGroup(group.id),
  ...
  const suggestCategory = async (desc: string) => {
    const categories = await queryClient.fetchQuery({
      queryKey: splitwiseQueryKeys.categories(),
      queryFn: () => splitwiseClient.getCategories(),
